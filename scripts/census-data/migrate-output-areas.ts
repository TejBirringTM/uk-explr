...
await client.transaction(async (client)=>{
    ...
    const nRowsInTable = await countRows(client, "uk-census-data", "output-areas");
    if (nRowsInTable > 0) {
        console.error(`The table must be empty before migration: "${dbName}"."uk-census-data"."output-areas"`);
        process.exit(1);
    }
});
