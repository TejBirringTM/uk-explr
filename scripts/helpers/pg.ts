...
export const userExists = async (client: PoolClient, userName: string) =>
  ((
    await client.query(
      `SELECT FROM pg_roles WHERE rolname='${userName}' AND rolcanlogin='true'`,
    )
  ).rowCount ?? 0) > 0;

export const roleExists = async (client: PoolClient, userName: string) =>
  ((
    await client.query(
      `SELECT FROM pg_roles WHERE rolname='${userName}' AND rolcanlogin='false'`,
    )
  ).rowCount ?? 0) > 0;
...
export const assertTableIsEmpty = async (
  dbName: string,
  schemaName: string,
  tableName: string,
) => {
